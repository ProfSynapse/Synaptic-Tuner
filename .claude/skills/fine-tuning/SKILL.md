...
- Save baseline eval scores before launching GRPO, including behavior/tool/environment suites when those are part of the target capability. If an eval process exits nonzero because test cases failed, still preserve and inspect `evaluation_results.md`, `evaluation_results.json`, and `stage_summary.json`.
- For repeat local eval on Windows, prefer `job.persist: true` and a stable `job.container_name` in the local eval YAML so Docker reuses the same copy-mode container and pip-installed runtime instead of accumulating stopped one-shot containers.
...
- For long-running local Docker training or eval, launch through the checked-in job config in a background process with redirected logs. Poll process state and logs with short commands; stop and relaunch when logs show an early configuration failure instead of waiting for a broad shell timeout.
- For local Docker smoke tests, keep the job self-cleaning: prefer bind mode when the model already exists on the host, set `persist: false` and `keep_container: false`, redirect logs to the repo artifact/log directory, and prune failed smoke containers/images before launching repeated attempts. Avoid broad `pip install --upgrade` in the base image; use an isolated venv and pin only the minimal packages needed, preferably with `--no-deps` when the image already has Torch/CUDA/Transformers.
- On Windows Docker Desktop, remember Docker's WSL VHDX lives on `C:` even when the repo is on another drive. After large failed Docker attempts, run `docker system df`, then `docker system prune -af --volumes` if the reclaimable size is high. Compact the Docker VHDX separately if Windows has not returned the freed space.
- When reusing persistent local eval containers, make model transfer explicit in job config. Use lightweight `setup.copy` lists for repeat evals after the model is already present, and recopy the model only when verification shows the container copy is missing or partial.
...
- Do not assume the local vLLM eval runtime is a valid in-process GRPO runtime. Eval serving, TRL trainer import, rollout helpers, model architecture support, and CUDA/Torch ABI compatibility are separate checks. For env-GRPO, smoke the exact three-way combination of trainer package, model/runtime Transformers support, and rollout backend before launching a long run.
- For env-GRPO, prove that the trainer is actually calling the configured rollout path before trusting aggregate rewards. Enable config-driven trajectory logging for smokes and verify per-rollout fields such as `env_passed`, `stop_reason`, `total_tool_calls`, and final text status. A metrics-only run can look healthy while silently using a normal generation path that never executes the environment.
- TRL releases may expose `rollout_func` in the trainer signature but only wire it through specific generation backends. If `use_vllm: false` with a direct Transformers rollout backend, verify that the run writes rollout trajectories and that reward kwargs include the extra rollout fields. If not, use or add a generic trainer bridge that honors any configured rollout function outside vLLM without hardcoding scenario/tool behavior.
- Environment assertions that only check static fixture state can false-pass before the model acts. For GRPO smokes, configure expected tool/action requirements from declarative fields such as an `expected_tools` list or a task-context path, and make the rollout pass those expectations into environment finalization. Keep the source field and transforms in YAML so different tool schemas remain config-driven.
- Treat `reward_std: 0` / `frac_reward_zero_std: 1.0` as "no GRPO learning signal" even when rewards are nonzero. Use trajectory logs to determine whether every sample is false-passing, every sample is failing the same way, or the model never emits actions. Scale only after a small smoke shows mixed outcomes within GRPO groups.
- If an env-GRPO smoke fails around TRL/vLLM imports, capture the full import traceback in the smoke logs before changing pins. A version that serves the model may still be unsupported by the installed TRL trainer, and a version accepted by TRL may still lack the model architecture or match the wrong Torch ABI.
- For new model families that require a newer Transformers stack than the base Unsloth image provides, first prove a non-vLLM env-GRPO smoke before trying colocated vLLM. A reliable pattern is: remove or hide incompatible vLLM packages in the ephemeral training container, install a coherent Transformers/TRL/HF Hub set, use a config-selected direct model generation backend, and keep vLLM reserved for evaluation/server use until a training server mode has been proven.
- Env-GRPO smokes on consumer GPUs should use config-driven PEFT/LoRA. If the trainer reaches the optimizer step and OOMs after loading the model, verify that the run YAML passes a LoRA/PEFT block through to the trainer instead of updating the full model.
- After a runtime stack is proven in an ephemeral smoke, bake it into a small checked-in Dockerfile under the local training image area and point the job YAML at a local tag with `job.pull_policy: never`. Keep the Dockerfile runtime-only: dependency pins, import checks, and environment variables are fine; scenario/tool/reward behavior stays in the run YAML.
- Treat vLLM image experiments as expensive on Windows Docker Desktop. Pull one candidate at a time, use `docker system df` between attempts, and remove losing image tags once a candidate is ruled out. Do not keep multiple large vLLM images around unless they are known-good active runtimes.
...
- Do not assume `unsloth/unsloth:latest` is a safe Qwen3.5 runtime. For Qwen3.5 local Docker smokes and training, use a checked-in job config with an explicit image tag/digest and stage-local dependency pins. If a smoke needs a newer TRL/OpenEnv layer, isolate it and avoid pulling a replacement Torch/CUDA stack into Docker.
- For env-GRPO with TRL OpenEnv support, prefer the documented TRL 0.27.x path over unpinned/current TRL. Do not upgrade `huggingface_hub` to `>=1.0` inside an Unsloth/Transformers runtime that requires `huggingface-hub<1.0`; if the image already has Torch/CUDA/Transformers/HF Hub, overlay only the missing TRL wheel with `--no-deps` for a smoke.
