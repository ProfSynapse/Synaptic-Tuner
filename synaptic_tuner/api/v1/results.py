...
class TrainingRunRef:
    ...
    @classmethod
    def from_dict(cls, value: dict[str, object]) -> "TrainingRunRef":
        value = exact_fields(value, frozenset({"run_id", "project_ref"}), "training_run_ref")
...
class VerifiedArtifact:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "size_bytes", exact_integer(self.size_bytes, "size_bytes"))

    def to_dict(self) -> dict[str, object]:
        return {"role": self.role, "sha256": self.sha256, "size_bytes": self.size_bytes}

    @classmethod
    def from_dict(cls, value: dict[str, object]) -> "VerifiedArtifact":
        value = exact_fields(value, frozenset({"role", "sha256", "size_bytes"}), "verified_artifact")
        return cls(
