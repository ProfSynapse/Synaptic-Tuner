...
operations, durable coordinator, retained Foundation boundary, and an inactive
provider registration. The caller may place
that registration in its own lazy registry; this function never mutates a global
registry and does not advertise public capability activation.
...
reads remain deliberately denied while the descriptor advertises inactive
capabilities.
