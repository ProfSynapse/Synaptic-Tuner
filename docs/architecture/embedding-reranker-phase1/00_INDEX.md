...
| **R6** | `frozen_head` nonstandard-pooling caveat | `frozen_head` mode requires a compare-to-plain-`SentenceTransformer`-baseline gate (a smoke assertion that frozen-head embeddings match the ST baseline within tolerance before the head is trained). | §2.4 |
