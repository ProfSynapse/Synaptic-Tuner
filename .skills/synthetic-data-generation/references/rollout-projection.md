...
The [rollout dataset projector](../../../SynthChat/scripts/project_rollout_datasets.py) turns saved environment-rollout artifacts into training datasets. It always writes canonical, KTO, and GRPO outputs, and can optionally write a full multi-turn SFT output. A config-driven filter engine lets you curate which rollouts reach each projection target.
...
The projector accepts `--filter-config <yaml>` pointing at a YAML file with a `projection.filters` list. The engine lives in the [generic rollout filter module](../../../shared/validation/rollout_filters.py) and filters arbitrary record dictionaries on any field via dot-path. It knows nothing about "turns" or "tools" specifically.
...
- Schema: [SFT projection schema](../../../Datasets/environment_rollouts/sft_projection.schema.json).

### Chat-template behavior

The [SFT preprocessor](../../../shared/sft_preprocessing.py) renders the `tool` role to **text** (re-tagged consistent with how the environment presents tool output to the model) at template time. Consequences:
