...
1. You MUST complete Steps 1-5 in order for every scenario or rubric change;
   only an explicitly identified plumbing smoke may omit judge-backed quality.
...
```bash
.skills/synthetic-data-generation/scripts/dry_run.sh YOUR_SCENARIO_KEY [count]
```
...
.skills/synthetic-data-generation/scripts/jsonl_to_markdown.sh \
...
After the sample is admitted and the user approves scaling, return to the route
that invoked this protocol. Preserve the dry-run artifacts and approval with the
full run record.
