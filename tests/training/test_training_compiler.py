"""Contract tests for engine-owned rich compilation contracts."""
...
def test_host_resolver_contract_is_internal_and_structural() -> None:
    class Resolver:
...
def test_compile_training_plan_owns_the_complete_planning_pipeline(
    tmp_path: Path,
) -> None:
...
def test_compile_training_plan_has_only_the_closed_keyword_contract() -> None:
...
def test_compile_training_plan_rejects_foreign_boundary_types(
    tmp_path: Path,
) -> None:
    training_input = _training_input()
    context = _planning_context(tmp_path)
...
def test_compile_training_plan_never_executes_dynamic_resolver_lookup(
    tmp_path: Path,
) -> None:
    dynamic_lookups = 0
...
def test_compile_training_plan_rejects_descriptors_without_execution(
    tmp_path: Path,
) -> None:
    descriptor_calls = 0
...
def test_compile_training_plan_rejects_callable_instance_attributes(
    tmp_path: Path,
) -> None:
...
def test_compile_training_plan_binds_before_the_one_deliberate_call(
    tmp_path: Path,
) -> None:
...
def test_compile_training_plan_rejects_a_foreign_service_result(
...
def test_compile_training_plan_eagerly_validates_the_fingerprint(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        TrainingPlan,
...
def test_compile_training_plan_symbol_import_is_provider_light() -> None:
    script = f"""
import json, sys
...
forbidden = sorted(name for name in after if (name.startswith('tuner.training.') and name != 'tuner.training.contracts') or name.startswith(('tuner.backends', 'tuner.execution.providers', 'docker', 'modal', 'huggingface_hub', 'runpod', 'sqlite3', 'torch', 'transformers', 'unsloth')))
...
def test_rich_records_and_config_are_immutable() -> None:
...
def test_planning_contract_rejects_a_provisional_source_lock() -> None:
...
def test_source_lock_binding_is_exactly_embedded_in_execution_source() -> None:
