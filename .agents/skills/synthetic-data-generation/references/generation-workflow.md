# Generation Workflow and Quality Gates
...
- [Environment execution configuration](../../../Evaluator/config/environment_execution.yaml) — Runtime action inference rules (config-driven)
- `Datasets/synthchat/` — Generated datasets go here (dry-runs and full runs)
- `SynthChat/interactions/` — Judge/improve logs

The mandatory small-sample gate is defined in
`../protocols/testing-before-scale.md`. Use the focused references routed from
`../SKILL.md` for command and authoring detail.
...
The [checked-in evaluator smoke](../../../configs/prompt_optimization/labkit_epistemic_humility_evaluator_smoke.yaml)
...
Profiles live in the [privacy profiles configuration](../../../SynthChat/config/privacy_profiles.yaml).

Global defaults live in [SynthChat settings](../../../SynthChat/config/settings.yaml)
under `privacy_preprocess`.
...
Key config files:
- [Tool-call formats](../../../SynthChat/config/tool_call_formats.yaml) — response schemas, wrapper name, context fields, and call structure.
- [Workspace formats](../../../SynthChat/config/workspace_formats.yaml) — system prompt sections and structure.
- [Label mappings](../../../SynthChat/config/label_mappings.yaml) — issue classification and label rollups.
- [SynthChat settings](../../../SynthChat/config/settings.yaml) — generation settings, model config, and output paths.
...
- For privacy smoke tests, prefer the checked-in fixtures under `tests/fixtures/privacy/`, the [privacy target manifest](../../../SynthChat/config/targets_privacy_docs_smoke.json), and the [privacy smoke runbook](../../../docs/plans/synthchat-privacy-smoke-runbook.md).
