...
- For new unsteered local generation, prefer `batch-generate --engine vllm`. Set `VLLM_BATCH_INVARIANT=1` before startup and pin `--expected-vllm-version`, the documented `--min-compute-capability`, model and tokenizer revisions, compute dtype, tensor parallel size, `max_num_seqs`, `max_num_batched_tokens`, structured-output backend, model context limit, multimodal limits, decode settings, and a JSON Schema when output formatting is incidental. The command records prompt-byte and prompt-token hashes rather than raw prompt token IDs.
- If a model generation config has an alternate EOS-like token that can end a structured response before its grammar is complete, use repeatable `--suppress-token '<exact-token-string>'` to prohibit only that token. Each value must resolve to one distinct non-EOS token. The pinned vLLM 0.23 path uses its internal `_bad_words_token_ids` sampling field to avoid raw/leading-space `bad_words` retokenization and fails closed if that field is unavailable. Configured strings and masked IDs are recorded in provenance. Canonical EOS suppression is rejected because it remains the normal grammar-completion stop.
