...
def extract_tool_info(response: str) -> Tuple[Optional[str], Optional[dict]]:
    """Extract tool name and arguments from a response string.
...
def group_by_prompt(examples: list[dict]) -> dict[str, dict]:
    ...
    for ex in examples:
        ...
        response = ex["conversations"][1]["content"]
        label = ex.get("label", True)

        if label:
            groups[prompt]["true"].append(response)
        else:
            groups[prompt]["false"].append(response)
